const moreProjectsData = [


    {
        keyMP: "mp1",
        idMP: "mp1",
        srcMP: ".././ImageUpload.png",
        titleMP: "titleMP1",
        descriptionMP: "descriptionMP1",
        featuresMP: ['Upload image/ title/ description', 'Add comments', 'Modal'],
        technologiesFEMP: ['JS', ' CSS', 'HTML', 'Vue.js'],
        technologiesBEMP: ['Node.js', 'Express', 'PostgreSQL', 'AWS (s3)', 'HTTP request'],
        hrefGitHubMP: "https://github.com/Edith2019/imageboard",
        herokuMP: "Heroku",
        hrefHerokuMP: "https://img-board.herokuapp.com/",
    },

    {
        keyMP: "mp2",
        idMP: "mp2",
        srcMP: ".././petition2.png",
        titleMP: "titleMP2",
        descriptionMP: "descriptionMP2",
        featuresMP: ['contact form', 'facebook event', 'translation'],
        technologiesFEMP: ['JS', ' CSS', 'HTML', 'Vue.js', 'bootstrap', 'smooth-scrolling', 'progressive-images', 'i18n-next'],
        technologiesBEMP: ['Node.js', 'Express', 'PostgreSQL', 'AWS (ses)', 'HTTP request'],
        hrefGitHubMP: "https://github.com/Edith2019/petition",
        herokuMP: "Heroku",
        hrefHerokuMP: "https://petitiontempelhof.herokuapp.com/"
    },

    {
        keyMP: "mp3",
        idMP: "mp3",
        srcMP: ".././connectfour.png",
        titleMP: "titleMP3",
        descriptionMP: "descriptionMP3",
        featuresMP: ['contact form', 'facebook event', 'translation'],
        technologiesFEMP: ['JS', ' CSS', 'HTML', 'Vue.js', 'bootstrap', 'smooth-scrolling', 'progressive-images', 'i18n-next'],
        technologiesBEMP: ['Node.js', 'Express', 'PostgreSQL', 'AWS (ses)', 'HTTP request'],
        herokuMP: "Github page",
        hrefGitHubMP: "https://github.com/Edith2019/connect-four",
        hrefHerokuMP: "https://edith2019.github.io/connect-four/",

    },

]


export default moreProjectsData;